...
# יצירת תיקייה לתמונות אם היא لا קיימת
...
# --- בניית ה-CSS הדינמי המותאם בצורה מושלמת למובייל ---
...
/* העלמת אלמנטים מיותרים באקורדיון */
details > summary {{
...
/* כרטיסיות מותאמות למובייל כך שלא יחרגו מהמסך */
...
/* כפתורים רוחביים נוחים ללחיצה באצבע */
.stButton > button {{
...
/* התאמת אינפוטים למובייל */
input {{
    font-size: 16px !important; /* מונע זום אוטומטי מעצבן באייפון בזמן הקלדה */
}}
</style>
...
def match_card_html(date, competition, stadium, home_team, away_team, score, home_logo, away_logo, league_logo, theme_name, attended=False):
    ...
    img_league = f"<img src='{league_logo}' width='18' style='vertical-align: middle; margin-left: 4px; border-radius: 50%;'>" if league_logo else ""
    img_home = f"<img src='{home_logo}' width='30' style='vertical-align: middle; margin-left: 6px;'>" if home_logo else ""
    img_away = f"<img src='{away_logo}' width='30' style='vertical-align: middle; margin-right: 6px;'>" if away_logo else ""
    
    att_tag = "<br><span style='background: linear-gradient(45deg, #28a745, #20c997); color: white !important; padding: 2px 8px; border-radius: 12px; font-size: 0.7em; font-weight: 900; display: inline-block; margin-top: 4px;'>🎟️ באצטדיון</span>" if attended else ""
    
    return f"""
    <div class='match-card'>
        <div style='text-align: center; color: #888 !important; font-size: 0.8em; font-weight: bold; margin-bottom: 8px;'>
            📅 <span style='color: {tc_inline};'>{date}</span> &nbsp;|&nbsp; 🏆 {img_league} {competition} &nbsp;|&nbsp; 🏟️ {stadium} {att_tag}
        </div>
        <div style='text-align: center; font-size: 1.1em; display: flex; align-items: center; justify-content: center; color: {tc_inline}; font-weight: 900; flex-wrap: wrap; gap: 4px;'>
            {img_home} <span>{home_team}</span> 
            <span style='background: linear-gradient(135deg, #007bff, #0056b3); color: white !important; padding: 3px 12px; border-radius: 16px; font-weight: 900; margin: 0 6px; font-size: 0.85em; letter-spacing: 1px;'>{score}</span> 
...
if nav_choice == "📋 יומן המשחקים":
    if len(st.session_state.saved_matches) > 0:
        ...
        st.markdown(f"<p style='color: gray !important; font-size: 0.85em; font-weight: bold;'>מציג {len(filtered_matches)} מתוך {len(st.session_state.saved_matches)} משחקים שמורים</p>", unsafe_allow_html=True)
        ...
        if team1_name.strip() == "" or team2_name.strip() == "":
            ...
            with st.spinner('מחפש קבוצות בשרת...'):
                ...
                if res1 == "DAILY_LIMIT" or res2 == "DAILY_LIMIT":
                    ...
                    st.info("לא מצאנו את הקבוצות. נסה לכתוב רק מילה אחת באנגלית (כמו Hilal).")
                    st.session_state.t1_opts = []
                    st.session_state.t2_opts = []
                else:
