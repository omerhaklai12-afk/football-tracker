...
def delete_match_from_file(match_id):
    ...
    df = pd.DataFrame(current_data)
    if df.empty:
        df = pd.DataFrame(columns=["ID_משחק", "תאריך", "תחרות", "מארחת", "תוצאה", "אורחת", "אצטדיון", "הייתי_במשחק", "פרטים_מלאים_משחק"])
    else:
        df = df.sort_values(by='תאריך', ascending=False)
    df.to_csv(CSV_FILE, index=False)
    st.session_state.saved_matches = df.to_dict('records')
    ...
    if len(st.session_state.saved_matches) > 0:
        ...
        for idx, match in enumerate(filtered_matches):
            ...
            with col_match:
                ...
                with st.expander("📊 פרטי המשחק וזיכרונות מהיציע"):
                    st.markdown(f"<div style='text-align: center; font-weight: bold; color: gray;'>מסגרת: {competition} | אצטדיון: {stadium}</div>", unsafe_allow_html=True)
        ...
        if team1_name.strip() == "" or team2_name.strip() == "":
            ...
            with st.spinner('מחפש קבוצות בשרת...'):
                ...
                if res1 and res2:
                    ...
                    st.warning("⚠️ לא נמצאו תוצאות לקבוצות שהוזנו.")
                    st.session_state.t1_opts = []
                    st.session_state.t2_opts = []
